...
\end{center}
\begin{center}
    \textbf{\large \textbf{学号：21121178 \quad 姓名： \quad 指导教师：刘洋}}
\end{center}
\hrulefill
\section{实验环境}
\begin{itemize}
...
select count(*) from titanic where Survived=1;
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.4\textwidth]{res1.png}
        \end{figure}
        \item 存活/未存活的男女数量及比例？
        \begin{lstlisting}[style=sqlstyle]
...
GROUP BY 
    Pclass;
        ...
        \begin{lstlisting}[style=sqlstyle]
SELECT Age_Group,COUNT(CASE WHEN Survived = 1 THEN 1 ELSE NULL END) AS Survived_Count,
    COUNT(*) AS Total_Count,
    ROUND((COUNT(CASE WHEN Survived = 1 THEN 1 ELSE NULL END) * 100.0 / COUNT(*)), 2) AS Survival_Rate
FROM 
    (SELECT 
