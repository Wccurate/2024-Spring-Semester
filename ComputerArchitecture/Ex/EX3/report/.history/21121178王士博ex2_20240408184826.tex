...
\section{实验感想}
通过这次实验，我对GPU计算的潜力和并行计算的效率有了更深刻的认识。实验数据清楚地表明，GPU在处理矩阵乘法这类高度并行化的任务上，相较于传统CPU，具有显著的加速优势。在解析和比较各种数据时，我被GPU架构带来的硬件优势和内存带宽的高效利用所震撼。CUDA技术真正实现了通过并行处理大幅度减少计算时间的可能性，而这一点在规模更大的矩阵乘法中表现得尤为明显。
